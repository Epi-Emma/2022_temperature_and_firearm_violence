...
################################################################
## DLNM - Maximum Daily Heat and Firearm Injuries (0-Day Lag) ##
################################################################
...
for(i in seq(length(dat_list))) {
  ...
  argvar <- list(fun=varfun,knots=quantile(data$Tmax_C,varper/100,na.rm=T),
                 degree=vardegree)
  cb <- crossbasis(data$Tmax_C,lag=0,argvar=argvar)
...
# TEST THE EFFECTS
fwald(mv,"avgTmax_C") #p=0.01
fwald(mv,"rangeTmax_C") #p=0.08
...
#write.csv(export, file = "[DIRECTORY PATH]/City_Results_0DayLag.csv",
#          na = "", row.names = TRUE)
...
tiff("[DIRECTORY PATH]/Pooled_Assoc_0DayLag.tiff", 
...
pdf("[DIRECTORY PATH]/DLNM_0DayLag_City_Graphs.pdf",width=8,height=9)
